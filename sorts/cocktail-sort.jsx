...
export default function CocktailSortVisualizer() {
  ...
  async function cocktailSort(a, cmp, swp) {
    const n = a.length;
    let swapped = true;
    let start = 0;
    let end = n - 1;
    ...
    while (swapped) {
      ...
      // Forward pass
      setDirection("forward");
      addLog("Forward pass: moving largest element to end", "system");
      swapped = false;
      
      for (let i = start; i < end; i++) {
        if (stopRef.current) return;
        updateColors({ [i]: "compare", [i + 1]: "compare" });
        cmp(); addLog(`Comparing a[${i}]=${a[i]} and a[${i+1}]=${a[i+1]}`);
        await delay();
        
        if (a[i] > a[i + 1]) {
          [a[i], a[i + 1]] = [a[i + 1], a[i]];
          swp(); updateArr(a);
          addLog(`Swapped → a[${i}]=${a[i]}, a[${i+1}]=${a[i+1]}`, "swap");
          swapped = true;
          await delay();
        }
      ...
      updateColors({ [end]: "sorted" });
      end--;
      
      // Backward pass
      setDirection("backward");
      addLog("Backward pass: moving smallest element to beginning", "system");
      swapped = false;
    ...
    <div style={{
      ...
      <div style={{
        ...
        <p style={{ color: "#64748b", fontSize: "14px", margin: "8px 0 0 0", textAlign: "center" }}>
          Time: O(n²) | Space: O(1) | Stable: Yes | Bidirectional Bubble Sort
        </p>
        {running && (
          <p style={{ color: "#f59e0b", fontSize: "12px", margin: "4px 0 0 0", textAlign: "center" }}>
