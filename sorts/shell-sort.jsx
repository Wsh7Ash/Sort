...
export default function ShellSortVisualizer() {
  ...
  async function shellSort(a, cmp, swp) {
    const n = a.length;
    ...
    for (let gap = Math.floor(n / 2); gap > 0; gap = Math.floor(gap / 2)) {
      ...
      for (let i = gap; i < n; i++) {
        ...
        addLog(`Inserting a[${i}]=${temp} with gap ${gap}`);
        
        // Shift earlier gap-sorted elements up until the correct location for a[i] is found
        for (j = i; j >= gap && a[j - gap] > temp; j -= gap) {
          if (stopRef.current) return;
          
          updateColors({ [j]: "compare", [j - gap]: "compare" });
          cmp(); addLog(`Comparing a[${j-gap}]=${a[j-gap]} and temp=${temp}`);
          await delay();
          
          a[j] = a[j - gap];
          swp(); updateArr(a);
          addLog(`Shifted a[${j-gap}] to position ${j}`, "swap");
          await delay();
        }
